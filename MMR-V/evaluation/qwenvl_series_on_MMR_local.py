...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--merge-outputs",
        action="store_true",
        help="Merge per-rank outputs into a single JSON file on rank 0.",
    )
...
def select_rank_samples(samples: List[dict], rank: int, world_size: int) -> List[dict]:
    if world_size <= 1:
        return samples
    return [sample for idx, sample in enumerate(samples) if idx % world_size == rank]


def merge_outputs(output_path: str, world_size: int) -> None:
    combined = []
    for rank in range(world_size):
        rank_path = f"{output_path}.rank{rank}"
...
def main() -> None:
    ...
    rank_samples = select_rank_samples(
        samples, accelerator.process_index, accelerator.num_processes
    )
    rank_output_path = (
        args.output_path
        if accelerator.num_processes == 1
        else f"{args.output_path}.rank{accelerator.process_index}"
    )

    iterator = rank_samples
    if accelerator.is_main_process:
        iterator = tqdm(rank_samples, desc="MMR-V")
    ...
    for idx, sample in enumerate(iterator):
        ...
        write_to_json(results, rank_output_path, indent=4)
    ...
    if (
        ...
        and args.merge_outputs
    ):
        merge_outputs(args.output_path, accelerator.num_processes)
