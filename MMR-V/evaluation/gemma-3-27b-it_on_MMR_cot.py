...
if __name__ == '__main__':
    ...
    if args.continue_eval:
        if os.path.isfile(save_file):
            ...
            results = [elem for elem in results if elem[f"{model_name}_raw_response"] is not None]
    ...
    client = OpenAI(
        model_version='/mnt/usercache/zhaosuifeng/model/gemma-3-27b-it/',
        api_type='openai',
        api_key="",
        api_url="http://210.75.240.155:25712/v1/chat/completions",
    ...
    for idx,sample in enumerate(samples[:]):
        ...
        if curr_id in id_set and id2sample[curr_id][f"{model_name}_raw_response"] is not None:
            continue
