...
if __name__ == '__main__':
    ...
    for path in file_paths:
        if os.path.exists(f"{path}/implicit_video_anonotations"):
            save_file = f'{path}/implicit_video_anonotations/results/{model_name}_on_MMR_V_cot.json'
    ...
    client = OpenAI(
        model_version='/mnt/usercache/zhaosuifeng/model/Phi-4-multimodal-instruct/',
        api_type='openai',
        api_key="",
        api_url="http://210.75.240.155:22345/v1/chat/completions",
    ...
    for idx,sample in enumerate(samples[:]):
        ...
        if isinstance(response, str):
            # 尝试 [[X]] 的所有匹配
            json_regex = r'\[\[([A-L])\]\]'
            all_answers = re.findall(json_regex, response)
            if all_answers:
                final_answer = all_answers[-1]
                sample[f"{model_name}_response"] = {"final_answer": final_answer}
                print(f"Extracted last answer: {final_answer}")
            else:
                # 回退到 \boxed{X}
                box_regex = r'\\boxed\{([A-L])\}'
                all_boxed = re.findall(box_regex, response)
                if all_boxed:
                    final_answer = all_boxed[-1]
                    sample[f"{model_name}_response"] = {"final_answer": final_answer}
                    print(f"Extracted last boxed answer: {final_answer}")
                else:
